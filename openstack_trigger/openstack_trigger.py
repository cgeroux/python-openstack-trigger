...
def addParserOptions(parser):
  ...
  parser.add_option("--list-metrics",dest="listMetrics"
    ...
    ,help="Only list available metrics and stop [not default]")
...
def processTriggers(xmlTriggers,options,path):
  ...
  for xmlTrigger in xmlTriggers:
    ...
    #get reference value
    reference=float(xmlTrigger.find("reference").text)
    comparison=xmlTrigger.find("comparison").text
    
    #get file containing actions to preform if trigger condition satisfied
    actionFile=xmlTrigger.find("action-file").text
